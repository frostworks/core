<ul component="category" class="category" itemscope itemtype="http://www.schema.org/ItemList" data-nextstart="{nextStart}" data-set="{set}">
	...
	<li component="category/topic" class="row clearfix category-item {function.generateTopicClass}" <!-- IMPORT partials/data/category.tpl -->>
		...
		<div class="col-md-7 col-sm-9 col-xs-10 content">
			<a class="icon" href="<!-- IF ../user.userslug -->{config.relative_path}/user/{../user.userslug}<!-- ELSE -->#<!-- ENDIF ../user.userslug -->" title="{../user.name}">
			...
			<h1 component="topic/header" class="title">
				...
				<small>
					<a href="{config.relative_path}/category/{topics.category.slug}"><span class="fa-stack"><i style="color:{topics.category.bgColor};"class="fa fa-circle fa-stack-2x"></i><i style="color:{topics.category.color};" class="fa {topics.category.icon} fa-stack-1x"></i></span> {topics.category.name}</a> &bull;
				</small>
		...
		<div class="col-md-3 col-sm-3 teaser hidden-xs" component="topic/teaser">
			<div class="card" style="border-color: {topics.category.bgColor}">
				...
				<small>
					<a href="{config.relative_path}/user/{topics.teaser.user.username}">
